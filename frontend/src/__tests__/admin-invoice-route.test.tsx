...
function renderInvoice() {
  ...
  const invoiceRoute = AdminInvoiceRoute.update({
  ...
  const router = createRouter({
    ...
    history: createMemoryHistory({ initialEntries: ['/admin/orders/ord-1/invoice'] }),
  });
  return render(
...
beforeEach(() => {
  vi.mocked(fetchAdminInvoice).mockReset();
});

describe('AdminInvoiceRoute', () => {
  it('renders a loading skeleton while the invoice is pending', async () => {
    vi.mocked(fetchAdminInvoice).mockReturnValue(new Promise(() => {}));
  ...
  it('shows a failure message when the invoice query errors', async () => {
    vi.mocked(fetchAdminInvoice).mockRejectedValue(new Error('boom'));
    renderInvoice();
  ...
  it('renders the invoice document with the invoice number once loaded', async () => {
    vi.mocked(fetchAdminInvoice).mockResolvedValue(invoice());
    renderInvoice();
  ...
  it('renders a back-to-order link to the admin order detail route', async () => {
    vi.mocked(fetchAdminInvoice).mockResolvedValue(invoice());
    renderInvoice();
  ...
  it('passes the order id from the URL through to fetchAdminInvoice', async () => {
    vi.mocked(fetchAdminInvoice).mockResolvedValue(invoice());
    renderInvoice();
    await waitFor(() => {
      expect(vi.mocked(fetchAdminInvoice)).toHaveBeenCalledWith('ord-1');
    });
  });
});
